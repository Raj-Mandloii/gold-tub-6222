import {
    Box, Button, Flex, Text, Image, NumberInput,useBoolean ,
    NumberInputField,
    NumberInputStepper,
    ...
    return (
        <Flex w={"100%"} direction={"column"} >
            ...
            <Flex justify={"center"} alignItems="center">
                <Image w={"22px"} src={"https://static.hsappstatic.net/ui-addon-upgrades/static-9.7120/js/img/MarketingHub_Icon.svg"} />
                <Text fontSize={"2xl"} color="rgb(51, 71, 91)" ml={"2%"} >{headOne}</Text>
            </Flex>
            <Box>
                <Text fontSize={"xs"} color="rgb(51, 71, 91)" fontWeight={"light"} >start at</Text>
                <Text fontSize={"lg"} color="rgb(51, 71, 91)" fontWeight={"600"} >{price}</Text>
                <Text fontSize={"xs"} color="rgb(51, 71, 91)" fontWeight={"light"} >billed monthly</Text>
                <Flex justifyContent={"center"}>
                <NumberInput step={1000} maxW={24} defaultValue={defaultValue} min={1000} >
                    <NumberInputField borderRadius="0" bgColor="#F5F8FA" fontSize={"12px"}/>
                    <NumberInputStepper>
                        <NumberIncrementStepper />
                        <NumberDecrementStepper />
                    </NumberInputStepper>
                </NumberInput>
                <Text fontSize={"sm"} mt="0.4em" ml="1em">Marketing Contacts</Text>
                </Flex>
                <Text fontSize={"xs"} color="gray">{marketing1}</Text>
                <Text fontSize={"xs"} color="gray">{marketing2}</Text>
                <Text color="gray" fontSize={"xs"}>{marketing3}<span className="underline" >More details.</span></Text>
                {/* <CustomSwitch/> */}
            </Box>
              ...
              {/*  **********************    SWITCH      ********************* */}
                <Flex width="75%" height={"50px"} bgColor={"gray.300"} m="auto" mt={"4%"}
                    ...
                    <Box ml="5%" width={"52%"} >
                        ...
                        <Text color="rgb(51, 71, 91)" fontSize={"xs"} fontWeight="light">Commit Annually</Text>
                    </Box>
            ...
            <Button border="2px" fontSize={"sm"}  backgroundColor={"#ff7a59"} color="white" width={"76%"} height="50px" mt="5%">{btnContent}</Button>
            <Box ml={"2em"} mt={"5%"}>
            <Text  textAlign={"start"} color="gray" fontSize={"sm"}><span className={link ? "underline" : ""}>{belowBtnContent}</span>{" "}plus:</Text>
    ...
    <Box>
      {employees.map((employee, index) => (
        <Box mb={"2em"} key={index}>
          <Text m={1} fontWeight={"bold"} fontSize={"sm"} textAlign={"start"} color={"#1F91AE"}>{employee.name}</Text>

          {employee.tasks.map((task, index) => (
            <Box display={"flex"} key={index}>
               <CheckIcon m={1}  color={"gray"} mt={"0.2em"} />
              <Text m={1} fontSize={"xs"} textAlign={"start"}>{task}</Text>
            </Box>
          ))}
